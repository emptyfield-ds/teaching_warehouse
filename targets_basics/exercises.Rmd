...
1. Run this code to create a new `_targets.R` file. Open the file to make sure it was created correctly.

```{r create-targets-r}
file.copy(
  "templates/_targets_yt2.R",
  "_targets.R",
  overwrite = TRUE
)
```
...
4. Confirm that `diabetes` has changed by looking at it with `tar_read()`

## Your Turn 4

1. Add `source("R/functions.R")` to `_targets.R`
2. Create a new target called `table_one` using `create_table_one(diabetes)`
3. Run `tar_visnetwork()` and `tar_make()`
4. Take a look at the target you just created
...
```{r create-targets-r}
file.copy(
  "templates/_targets_yt6.R",
  "_targets.R",
  overwrite = TRUE
)
```

2. Open `report.Rmd`. Which targets does this report depend on? Look for `tar_load()` and `tar_read()`.
3. Add a new target to generate your report. The format for for rendering a R Markdown file is `tar_render(name_of_target, "name_of_report.Rmd")`.
4.  Run `tar_visnetwork()` and `tar_make()`. Open your rendered report from the Files pane.
