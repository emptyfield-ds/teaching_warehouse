...
2. Use tidyr to `pivot_longer()` the variable names into one column and the values into another. Since you're passing two columns, you need to specify the `cols =` argument. Name the new columns `"key"` and `"value"` using `names_to` and `values_to`.
