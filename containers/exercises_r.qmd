...
- Add a `RUN` command to install R with rig: `rig add 4.5.1`
...
```
docker run --rm -v $(pwd)/figures:/workspace/figures my_r_container
```
...
```
docker run --rm -v $(pwd)/figures:/workspace/figures my_r_renv_container
```
...
```
docker run --rm -v $(pwd)/_targets:/workspace/_targets -v $(pwd)/figures:/workspace/figures my_r_targets_container
```
