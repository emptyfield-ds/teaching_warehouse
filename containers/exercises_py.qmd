...
- Initialize a uv project and install matplotlib and seaborn with the following commands:
...
```
docker run --rm -v $(pwd)/figures:/figures my_py_container
```
...
```
docker run --rm -v $(pwd)/figures:/figures my_py_uv_container
```
...
- Modify the `Dockerfile` from Your Turn 5 to include the following changes:
  - Copy the make-related files to the container:
...
```
docker run --rm -v $(pwd)/outputs:/outputs -v $(pwd)/figures:/figures my_py_make_container
```
