...
Now, we'll add another layer to the existing ggplot2 object (`gg_uk_recent_years`). For this layer, we'll use the same aesthetics but with this new data. Each geom inherits data from `ggplot()`, but you can use a different dataset using the `data` argument directly, in the geom.
