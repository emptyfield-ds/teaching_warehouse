...
uk_excess_deaths <- ft_excess_deaths %>% filter(# country == _____)
```
...
The first layer we will build is the lines for 'recent years', which we will call `uk_excess_deaths_prior` and it includes all years prior to 2020.
...
Our final plot will have `deaths` on the `y` axis, and `week`s across the `x`, so we'll calculate the average number of `deaths` per week and year by grouping the dataset by `week` and `year` and summarizing mean `deaths`. We will store the result in `uk_excess_deaths_priorWk`.

```{r create-uk_excess_deaths_priorWk}
uk_excess_deaths_priorWk <- uk_excess_deaths_prior %>%
  group_by(week, year) %>%
    summarize(deaths = # mean(______, na.rm = TRUE))
# view data 
head(uk_excess_deaths_priorWk)
```
...
uk_excess_deaths_priorWk <- uk_excess_deaths_priorWk %>%
...
```{r check-years}
ggplot(data = uk_excess_deaths_priorWk, 
...
gg_uk_recent_years <-  ggplot(data = uk_excess_deaths_priorWk, 
...
```{r create-uk_excess_deaths2020Wk}
# filter to the year 2020
uk_excess_deaths2020 <- uk_excess_deaths %>%
  filter(# year == )
# create uk_excess_deaths2020Wk
uk_excess_deaths2020Wk <- uk_excess_deaths2020 %>%
  # group by the week and year
  group_by(# week, # year) %>%
    # calculate mean deaths and mean expected_deaths
    summarize(deaths = # mean(, na.rm = TRUE),
...
gg_uk_excess_deaths <- gg_uk_recent_years +
    ...
    geom_line(data = uk_excess_deaths2020Wk,
...
gg_uk_total_excess_deaths <- gg_uk_excess_deaths +
    geom_ribbon(data = uk_excess_deaths2020Wk, aes(# ymin = , # ymax = ), 
...
The grouped data `uk_excess_deathsWk` is created in the code chunk below (be sure to note the `filter()` function before grouping and summarizing!)

```{r create-uk_excess_deathsWk}
uk_excess_deathsWk <- uk_excess_deaths %>%
...
Now we can plot the historical average, but we need to make sure we aren't inheriting any aesthetics from the previous layers (they are expecting a year variable we don't have in the `uk_excess_deathsWk` data), so we add `inherit.aes = FALSE` to the `geom_line()` function.
...
gg_uk_historical_average <- gg_uk_total_excess_deaths +
    geom_line(data = uk_excess_deathsWk, inherit.aes = FALSE, aes(# x = , # y = ),
             color = # set color)
...
Below is the code for reproducing the steps and creating the three data layers we built above. We end up with `ExcessDeathsPriorWk`, `ExcessDeaths2020Wk`, and `ft_excess_deathsWk`. Read through the comments for more information.
...
# historical averages -----------------------------------------------------
# these is the data from ft_excess_deathsWk (grouped only on country and week)
# remove the inherited aesthetics from the previous layers
    geom_line(data = ft_excess_deathsWk,
              # _____ = FALSE,
