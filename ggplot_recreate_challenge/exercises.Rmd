...
Below is the code for reproducing the steps and creating the three data layers we built above. We end up with `excess_deaths_prior_wk`, `excess_deaths2020_wk`, and `ft_excess_deaths_wk`. Read through the comments for more information.

```{r facet-data}
# create excess_deaths_prior_wk ----
# filter to correct date range
excess_deaths_prior_wk <- ft_excess_deaths %>%
  filter(year != # ) %>%
# excess_deaths_prior_wk ~~~ GROUP BY & SUMMARIZE deaths by week/year -------------
...
To create a plot similar to the one in the FT times, we'll need to duplicate each layer in the line graph for all 21 countries in the `excess_deaths_prior_wk` data. We can do this by adding the `ggplot2::facet_wrap()` function
...
# grouping variable
gg_facet_by_country <- ggplot(data = excess_deaths_prior_wk,
...
# total excess deaths -----------------------------------------------------
# here we have the area (ribbon) with the same excess_deaths2020_wk
...
# historical averages -----------------------------------------------------
# these is the data from ft_excess_deaths_wk (grouped only on country and week)
# remove the inherited aesthetics from the previous layers
    geom_line(data = ft_excess_deaths_wk,
              # _____ = FALSE,
