...
Use `dplyr::count()` to determine how many countries are in the `ft_excess_deaths` data.

```{r count-countries}
dplyr::count(x = ft_excess_deaths, # _______)
```
...
UKExcessDeaths <- ft_excess_deaths %>% dplyr::filter(# country == _____)
```
