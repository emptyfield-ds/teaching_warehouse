...
Use `count()` to determine how many countries are in the `ft_excess_deaths` data.

```{r count-countries}
count(x = ft_excess_deaths, # _______)
```
...
```{r latest-data, out.height="50%", out.width="50%", fig.align='center', echo=FALSE, eval=TRUE}
knitr::include_graphics("latest-data.png")
```

In order to build this graph, we will need to filter `ft_excess_deaths` to only the data for `"UK"` and store it in `UKExcessDeaths`. 

```{r create-UKExcessDeaths}
UKExcessDeaths <- ft_excess_deaths %>% filter(# country == _____)
```
...
We're going to build this graph from the `UKExcessDeaths`, and then apply what we've learned to the entire `ft_excess_deaths` dataset. 
