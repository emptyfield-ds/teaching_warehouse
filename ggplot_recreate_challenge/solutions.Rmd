...
The second layer will be mean deaths per week in 2020 (in otherwords, during the coronavirus pandemic). 
...
This time, we'll set some new aesthetics inside the new geom (`geom_ribbon(aes())`). `geom_ribbon()` takes two additional aesthetics: `ymin` and `ymax`.
...
gg_uk_total_excess_deaths <- gg_uk_excess_deaths +
  geom_ribbon(
    data = uk_excess_deaths2020_wk,
    aes(ymin = expected_deaths, ymax = deaths), 
    fill = "tomato", 
    alpha = 0.5
  )
