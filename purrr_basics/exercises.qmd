...
**Stretch goal**: Give the new columns the name pattern `column_function`, e.g. `bp.1s_mean` (hint: In the `.names` argument, `{.fn}` will give you the function name, and `{.col}` will give you the column name. See `?across`)
