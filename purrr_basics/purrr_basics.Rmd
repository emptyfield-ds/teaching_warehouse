...
gapminder %>% 
  group_by(continent) %>% 
    summarise(across(c("lifeExp", "gdpPercap"), median)) #<<
```
...
```{r slide-98-group_walk, eval=TRUE}
temp <- "temp"
fs::dir_create(temp)
gapminder %>%
  group_by(continent) %>%
  group_walk(~ write_csv(x = .x, file = file.path(temp, 
    paste0(.y$continent, ".csv"))))
```

```{r slide-98-group_walk-02, , echo = FALSE, eval=FALSE}
temp <- "temp"
fs::dir_create(temp)
gapminder %>%
  group_by(continent) %>%
  group_walk(write_csv, x = ., file = file.path(temp, 
    paste0(".csv")))
```

--

```{r slide-98-temp-dir_tree, echo=FALSE}
fs::dir_tree(temp, pattern = "csv$")
```

```{r slide-98-remove-temp, include=FALSE}
