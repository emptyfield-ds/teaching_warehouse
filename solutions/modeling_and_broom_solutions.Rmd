...
model1 <- diabetes %>% 
  ...
  lm(glyhb ~ waist_hip + gender, data = .)
...
_Bonus_: Add a regression line with geom_smooth(method = "lm"). Remember that with ggplot2, you add functions with `+` and not `%>%`
...
mod_wh %>% 
  augment(data = drop_na(diabetes, glyhb, waist_hip, gender, age)) %>% 
...
diabetes %>% 
  ...
  glm(diabetic ~ waist_hip + age + gender, family = binomial, data = .) %>% 
  tidy() %>% 
