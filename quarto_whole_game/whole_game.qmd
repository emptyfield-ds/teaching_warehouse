...
These data were collected as part of a study to estimate the prevalence of obesity, diabetes, and other cardiovascular risk factors in central Virginia for African-Americans.
...
description <- split(description, description$diabetic)
```
...
Non-diabetic participants were, on average, younger than those with diabetes (`r description$Healthy$age` vs `r description$Diabetic$age`).
There were `r description$Missing$n` participants who were missing data.
These participants were also younger, on average, than those with diabetes (`r description$Diabetic$age` vs `r description$Missing$age`).
See below for more.
...
# add labels to `diabetes`
var_label(diabetes) <- list( 
...
diabetes |> 
  ...
  _______(by = diabetic, missing_text = "(Missing)") |> 
  _______()
```
...
In \_\_\_\_\_\_\_ (a), we show the unadjusted relationship between A1c and waist-hip ratio (WH ratio is shown on the log scale).
As WH ratio increases, A1c also increases.
In \_\_\_\_\_\_\_ (b), we show the density of WH ratio among those with and without diabetes.
...
In \_\_\_\_\_\_\_, we present two regression models: linear and logistic.
