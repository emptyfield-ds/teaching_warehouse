...
Check `exercises.Rmd` for guidance on creating this report!
...
```{r forest-map, fig.cap="Net Forest Conversion in 2010", fig.width = 9}
tar_read(forest_map)
```

Figure \@ref(fig:forest-map) presents a map of net forest conversion in 2010, as well as the 10 countries with the highest absolute change in forest.
...
tar_read(brazil_causes_table)
```

Table \@ref(tab:causes-table) shows the causes of deforestation in Brazil in 2010.
...
```{r causes-hist, fig.cap="Causes of deforestation in Brazil over time, sorted by maximum loss", fig.width = 9}
tar_read(brazil_causes_hist)
```

Figure \@ref(fig:causes-hist) shows that, while deforestation has decreased in Brazil since the early 2000s, it remains steady at over 1 million hectacres lost per year.
Pasture clearing is consistently the top cause, while fire---generally being accidental---varies from year to year.

## Discussion

We've combined the magic of R Markdown for generating reports with the reliability of targets for managing our code, when we render the report, and understanding what targets the report depends on.
