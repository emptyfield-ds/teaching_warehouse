...
```{r}
#| label: filter-concept-map-example-comparisons
#| eval: false
filter(<DATA>, <PREDICATES>)
```
...
```{r}
#| label: filter-concept-map-example-operators
#| eval: false
filter(<DATA>, <PREDICATES>)
```
...
```{r}
#| label: filter-concept-map-example-in
#| eval: false
filter(<DATA>, <PREDICATES>)
```
...
```{r} 
#| label: filter-equal-example-01-highlight
#| highlight.output: !expr c(1, 4:7)
filter(diamonds, cut == "Ideal", carat > 3)
```
...
```{r}
#| label: mutate-example-highlight
#| highlight.output: 15.0
mutate(diamonds, log_price = log(price), log_pricesq = log_price^2)
```
...
```{r}
#| label: mutate-select-filter-challenge-highlight
...
```{r}
#| label: group-by-example-01-output-highlight
#| highlight.output: 2.0
diamonds |> 
  group_by(cut)
```
...
```{r}
#| label: group-by-summarize-example-01-output
diamonds |> 
  group_by(cut) |> 
  summarize(n = n(), mean_price = mean(price))
```
...
```{r}
#| label: group-by-mutate-example-01
#| eval: false
diamonds |> 
  group_by(cut) |> 
...
```{r}
#| label: group-by-mutate-example-01-output
...
```{r}
#| label: mutate-across-example-output
#| highlight.output: !expr c(15, 16)
mutate(
  diamonds, 
  across(c("carat", "depth"), list(sd = sd, mean = mean))
)
...
```{r}
#| label: mutate-across-where-example-output
#| highlight.output: !expr c(15, 16)
mutate(
  gapminder, 
  across(where(is.numeric), list(mean = mean, median = median)) #<<
