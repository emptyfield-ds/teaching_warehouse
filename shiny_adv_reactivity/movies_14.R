  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel(
          "Files in directory",
          DT::dataTableOutput(outputId = "csv_files")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  # Write sampled data as csv ---------------------------------------
  observeEvent(eventExpr = input$write_csv, handlerExpr = {
    filename <- paste0(
      "movies_",
  ...
  diff <- reactive({
    round(difftime(now(), beg, units = "secs"))
  })
