...
knitr::include_graphics("img/where_are_we_06.png")
```
...
```{r, eval=FALSE}
library(fs)
dir_create(path("inst", "templates"))
file_create(path("inst", "templates", "config.yml"))
```
...
#### Re-build your package and run `create_analysis()` in the console. If something went wrong, just delete the folder `avalanche_analysis` and try again.
...
## ~~Source files with `inst/scripts` and `sys.source()`~~
## *Add citations* to `inst/CITATION`
