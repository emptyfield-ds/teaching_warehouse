...
# here, this will give us the same results
# for the multiple imputation algorithm we'll use
set.seed(1234)
```

## Playing the whole game

We're now going to put these tools together to import and clean a data set, explore and model the data, and present the results. Our end goal is to create a model that predicts survival in passengers who were on the Titanic. When you're done, render the results.
...
Let's look at how many people survived. `survived` is stored as 0 (died) and 1 (survived), so let's first transform them to factors (see the chapter on factors in [R for Data Science](https://r4ds.had.co.nz/factors.html)). We'll also turn any of the variables labels (from the `.dta` file) in the data into factors.

Then, summarize how many people survived and how many died. You can use dplyr to group by survival and then summarize with `n()` or use the `table()` function on `titanic$survived`. 
...
3. This chunk can take a while to run. Set the code chunk option `cache: true` for this one. `cache` saves the results so you don't need to keep re-rendering the document. 
...
What's going on with this model? Something looks wrong. Try to fix it. Hint: when variables on the right hand side of the formula are highly correlated with each other (collinear), it can sometimes mess up the model fit. This often shows itself through extremely high standard errors. The solution is usually to remove one of the two.
