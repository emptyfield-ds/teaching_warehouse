...
We have several files containing the  `diabetes` dataset (it's the same data but in CSV, SAS, Stata, etc, formats).
...
It's very common to demarcate missing values with some code, like "-99" or "9999". The `na` option will replace such values with `NA`, the R object type for missing values.
