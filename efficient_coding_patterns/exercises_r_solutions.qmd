...
fd_calls_dt[, log_delay := log(Delay)][year == 2015]

fd_calls_dt[year == 2015][, log_delay := log(Delay)]
```
...
```{r}
transform_then_filter_tv <- function() {
  fd_calls |>
    mutate(log_delay = log(Delay)) |>
    filter(year == 2015)
}

filter_then_transform_tv <- function() {
  fd_calls |>
    filter(year == 2015) |>
    mutate(log_delay = log(Delay))
}
...
bench::mark(
  transform_then_filter_tv(),
  filter_then_transform_tv(),
  relative = TRUE
)
```
...
```{r}
transform_then_filter_dt <- function() {
  fd_calls_dt2 <- fd_calls_dt[, log_delay := log(Delay)][year == 2015]
  fd_calls_dt2
}

filter_then_transform_dt <- function() {
  fd_calls_dt2 <- fd_calls_dt[year == 2015][, log_delay := log(Delay)]
  fd_calls_dt2
}

bench::mark(
  transform_then_filter_dt(),
  filter_then_transform_dt(),
  relative = TRUE
)
```
...
population$age_cost <- age_effects$cost[match(population$age, age_effects$age)]
population$condition_cost <- condition_effects$cost[match(population$condition, condition_effects$condition)]

population$cost_lookup <- population$age_cost + population$condition_cost + noise

identical(population$cost_for_loop, population$cost_lookup)  
```

```{r}
for_loop <- function() {
  cost_for_loop <- vector("numeric", length = nrow(population))
  ...
  for (i in seq_len(nrow(population))) {
    ...
    cost_for_loop[[i]] <- age_cost + condition_cost + noise[[i]]
  }
...
vectorized <- function() {
  age_cost <- age_effects$cost[match(population$age, age_effects$age)]
  condition_cost <- condition_effects$cost[match(population$condition, condition_effects$condition)]
  
  cost_lookup <- age_cost + condition_cost + noise
...
```{r}
optimized_sum_squared_diffs <- function(mat) {
  col_means <- colMeans(mat)
  result <- colSums((mat - rep(col_means, each = nrow(mat)))^2)
...
bench::mark(
  sum_squared_diffs(mat),
  optimized_sum_squared_diffs(mat),
  relative = TRUE
)
```
...
```{r}
boot_medians <- future_vapply(
  seq_len(times), 
  bootstrap_median, 
  FUN.VALUE = numeric(1),
  # because we're generating random samples in each call
  future.seed = TRUE
)
