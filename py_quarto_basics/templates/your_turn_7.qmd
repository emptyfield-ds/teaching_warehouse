...
2.  Fill in the column name to use `.select()` and `to_numpy()` to get the the number of unique `year`s in gapminder as a scalar.
...
n_years = (gapminder
  ...
  .select(pl.col("year").unique().len())
  .to_numpy()[0, 0]
)
```

Gapminder observed data over `{python} n_years` different years.
