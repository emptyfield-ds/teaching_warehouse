...
These data were collected as part of a study to estimate the prevalence of obesity, diabetes, and other cardiovascular risk factors in central Virginia for African-Americansn[@Willems1997].
...
```{r figure-one, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "A1c and diabetes status by WH ratio"}
library(patchwork)
...
In Figure \@ref(fig:figure-one)a, we show the unadjusted relationship between A1c and waist-hip ratio (WH ratio is shown on the log scale).
As WH ratio increases, A1c also increases.
In Figure \@ref(fig:figure-one)b, we show the density of WH ratio among those with and without diabetes.
Participants with diabetes had a higher WH ratio than those without diabetes.

```{r table-two, eval = TRUE, echo = FALSE}
